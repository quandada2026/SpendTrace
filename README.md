...
- **v1.0**（已打 tag）：OCR 容错重构（多候选金额+打分+人工复核闸门）+ 多图上传 + 日历补记/翻月热力 + 上传日期归属上下文 + 统计按月联动 + 分类关键词字典扩充 + 日期识别增强 + 复核页日期选择器。
- 当前 `main` HEAD 在 v1.0 之后持续迭代，每版均通过本机 `./gradlew assembleDebug` 实测产出 APK。
